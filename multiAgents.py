...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        val = 0;
        #food calc
        FoodDist = [manhattanDistance(newPos, fd) for fd in newFood.asList()]
        FoodDist.sort()
        
        if len(FoodDist)>2:
            val -= FoodDist[0]*10+FoodDist[1]*5+FoodDist[2]*2
        elif len(FoodDist)>1:
            val -= FoodDist[0]*10+FoodDist[1]*5
        elif len(FoodDist)>0:
            val -= FoodDist[0]*10

        #Ghost calc
        GhostDist = [manhattanDistance(newPos, gt.getPosition()) for gt in newGhostStates]
        GhostDist.sort()
        if len(GhostDist)>1:
            val += GhostDist[0]*20+GhostDist[1]*5
        elif len(GhostDist)>0:
            val += GhostDist[0]*20

        #Scare calc
        newScaredTimes.sort()
        if newScaredTimes[0]+2<GhostDist[0]:
            val += GhostDist[0]*50
